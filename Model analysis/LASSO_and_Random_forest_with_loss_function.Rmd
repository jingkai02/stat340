...
2) We generated 3 lists of variables that were deemed significant. These are based on LASSO (using logit and complimentary log log link functions) and also ordinal random forest. We can then fit these variables to the ordinal logistic regression model to assess accuracy. Refer to the 
...
We see that the model is mostly concentrated around low values, we use the negative log-log link function, as recommended in https://www.bookdown.org/chua/ber642_advanced_regression/ordinal-logistic-regression.html 
...
(spoiler alert) We will discover below that the best model uses "equal" as the performance function. Hence, we will look at the relative importance of each variable in predicting alcohol consumption in the model. We use the Permutation variable importance of the variables, shown below, and they are ordered based on the misclassification score.
...
Hence, a possible next step would be to plug these 12 variables into our ordinal regression model.
...
#### Create a function to help us run our random forests tests on a specified train/test split (default is 80/20)

run_random_forests <- function(train_prop = 0.8){
  # Create dummy variable for splitting data
  
  split_dummy <- sample(c(rep(0, train_prop * nrow(alcohol)),
                          rep(1, (1-train_prop) * nrow(alcohol))))
  
  
  alcohol_train <- alcohol[split_dummy == 0,]
  alcohol_test <- alcohol[split_dummy == 1,]
  ...
  errors <- c(mean_dev1, mean_dev2)
  return(errors)

}
```

We now proceed to run n iterations of the train/test split, and average the errors across all n times to arrive at a final accuracy measure.

n can be adjusted lower to reduce the running time - here, we have chosen n = 5

```{r}
n = 5
error_randomforest1 <- rep(NA,n)
error_randomforest2 <- rep(NA,n)

for(i in 1:n){
  # each iteration takes around 37 seconds, so don't panic if this takes some time (around 5 mins) to run
  errors <- run_random_forests()
...
As observed above, the error using the 'equal' performance function is the best, so we will compare that model with the model
