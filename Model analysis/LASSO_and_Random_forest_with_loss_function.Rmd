...
2) We generated 4 lists of variables that were deemed significant. These are based on LASSO (using logit and complimentary log log link functions) and also ordinal random forest (using "equal" and "proportional" performance functions). We can then fit these variables to the ordinal logistic regression model to assess accuracy. Refer to the corresponding sections for data.
...
for(i in 1:n){
  # each iteration takes around 37 seconds, so don't panic if this takes some time (around 3 mins) to run
