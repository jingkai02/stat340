...
8) `Fedu`   - The student's fother’s education level, with 0 being non-educated, and 4 being in higher/tertiary education.
9) `Mjob`   - The student's mother's current employment position and status in the workforce (categorical variable)
10) `Fjob`  - The student's father's current employment position and status in the workforce (categorical variable)
11) `reason`
12) `guardian`
13) `traveltime`
14) `studytime`
...
```{r}
reg = lm(Dalc~reason, data=students)
summary(reg)
```

```{r}
ggplot(data=students, mapping=aes(x=reason, y=Dalc)) +
  geom_boxplot()
