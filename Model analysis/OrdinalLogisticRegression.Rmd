...
```{r}
model_RF1 <- polr(Walc ~ .-sex-goout-G3-absences-G2-studytime-age-Fjob-failures-G1-famsup-paid-Dalc-Walc_bin, data = df_recoded_binary_80, Hess=TRUE)
...
```{r}
model_RF2 <- polr(Walc ~ .-sex-goout-G3-absences-G2-studytime-age-Fjob-failures-G1-famsup-romantic-reason-Dalc-Walc_bin, data = df_recoded_binary_80, Hess=TRUE)
...
```{r}
summary_table <- coef(summary(model_log_log_link))
...
df_recoded_binary_fit_models <- function(holdout_idxs, K) {
  ...
  m4 <- polr(Walc ~ .-sex-goout-G3-absences-G2-studytime-age-Fjob-failures-G1-famsup-paid-Dalc-Walc_bin, data = train_data, Hess=TRUE)
  m4_pred <- predict(m4, leftout)
  errors[4] <- mean((as.numeric(m4_pred) - as.numeric(leftout$Walc))^2)
  
  # fit model 5, then evaluate
  m5 <-  polr(Walc ~ .-sex-goout-G3-absences-G2-studytime-age-Fjob-failures-G1-famsup-romantic-reason-Dalc-Walc_bin, data = train_data, Hess=TRUE)
